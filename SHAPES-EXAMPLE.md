...
The Approov integration is available via [`jitpack`](https://jitpack.io) or Maven. This allows inclusion into the project by simply specifying a dependency in the `gradle` files for the app. 
If you prefer using Maven you do not need to add external repository reference. If you prefer using `jitpack` , the `jitpack` reference needs to be added as follows to the end the `repositories` section in the `build.gradle:20` file at the top level of the project:
